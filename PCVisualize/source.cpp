...
// Vectors to store pointcloud data, translation vectors and Rotation matrices
vector< vector<float> > pointCloud;
vector< vector<double> > cVector;
vector< vector< vector<double> > > Rvector;
...
// Read pointCloud data from structure.txt file
...
// Read camera extrinsics from camera.txt
void readCameraExtrinsics(void)
{
	...
	while(getline(file, line))
	{
		...
		{
			float val;
			// The first line contains t vector
			if(lineCount%5 == 1)
			{
				for(int i=0; i<3; i++)
				{
...
{
	int camCount = cVector.size();


	for(int i=0; i<camCount; i++)
	{
		glPushMatrix();
		glTranslatef(cVector[i][0], cVector[i][1], cVector[i][2]);
		glutSolidCube(0.1f);
		glPopMatrix();
	}
...
void display() 
{
	...
	{			
		drawPoint(pointCloud[i][0], pointCloud[i][1], pointCloud[i][2], 
			pointCloud[i][3], pointCloud[i][4], pointCloud[i][5]);
	}
